...
namespace util {
    ...
    std::string get_multiline(std::vector<std::string>& history, const std::string& prompt_string = "> ",
                              const std::optional<std::function<bool(Term::Model&)>>& ctrl_c_callback = std::nullopt);
    void ignore_line();
    ...
    std::unique_ptr<Term::Terminal> initialize_or_throw(const bool& clear_screen = false, const bool& disable_signal_keys = false,
